...
export function TradeFeesRow(p: Props) {
  ...
  const priceImpactRebatesInfo = useMemo(() => {
    ...
    return (
      <Trans>
        ...
        <ExternalLink href={"https://docs.0xmarkets.io/docs/trading/v2#price-impact-rebates"} newTab>
          Read more
        </ExternalLink>
      </Trans>
    );
  ...
  const swapRouteMsg = useMemo(() => {
    ...
    return (
      <>
        <br />
        <Trans>This swap is routed through several 0xM pools for the lowest possible fees and price impact.</Trans>
      </>
    );
  }, [p.swapFees]);
